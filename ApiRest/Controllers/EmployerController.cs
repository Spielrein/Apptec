...
namespace ApiRest.Controllers
{
    ...
    public class EmployerController : ApiController
    {
        ...
        public IHttpActionResult Create([FromBody]EmployerModel employer)
        {
            ...
            var consulta = EmployerData.Crear(employer.Nombre, employer.Apellidop, employer.Apellidom, employer.Rfc, employer.RolesId, employer.InstitucionId,employer.User);
            return Ok(consulta);
        }
        ...
        [HttpPost]
        [Route("Show")]
        public IHttpActionResult Show(EmployerModel employer)
        {
            u = credenciales.getUsuario();
            c = credenciales.getUsuario();
            var consulta = EmployerData.Mostrar(employer.User);
            return Ok(consulta);
        }
        ...
        public IHttpActionResult Update(EmployerModel employer)
        {
            u = credenciales.getUsuario();
            c = credenciales.getUsuario();
            var consulta = EmployerData.Actualizar(employer.EmployerId, employer.Nombre, employer.Apellidop, employer.Apellidom, employer.Rfc, employer.RolesId, employer.InstitucionId,employer.User);
            return Ok(consulta);
        }

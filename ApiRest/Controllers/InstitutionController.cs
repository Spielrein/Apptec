...
namespace ApiRest.Controllers
{
    ...
    public class InstitutionController : ApiController
    {
        ...
        [HttpPost]
        [Route("SHow")]
        public IHttpActionResult Show(InstitutionsRegisterModel institutionsRegister)
        {

            u = credenciales.getUsuario();
            var consulta = InstitutionData.Mostrar(institutionsRegister.User);
            return Ok(consulta);
        }
        ...
        {
            u = credenciales.getUsuario();
            var consulta = InstitutionData.Crear(inst.Name, inst.Direction, inst.Phone, inst.EducationLevelID, inst.Director, inst.User);
            return Ok(consulta);
        }
        ...
        {
            u = credenciales.getUsuario();
            var consulta = InstitutionData.UpdateRegister(inst.InstitutionID, inst.Name, inst.Direction, inst.Phone, inst.EducationLevelID, inst.Logo, inst.Director, inst.User);
            return Ok(consulta);
        }
